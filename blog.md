...
| Association Rules | Lift (top 1) | **8,168431** | Support **(không có số trong ảnh)** |
| Clustering | K chọn để chạy | **3** | Theo Elbow plot (inertia) |
| Clustering | Failure_Rate_% cao nhất | **5,241730% (Cluster 0)** | Profiling cụm |
| Classification | ROC-AUC | **0,9842** | PR-AUC **(không có số trong ảnh)** |
...
| Forecasting | MAE / RMSE | **4,3920 / 21,8645** | SMAPE **(không có số trong ảnh)** |
...
## Cấu trúc repository và mô tả module `src/`
...
**Gợi ý điền placeholder nhanh:** chạy notebook và in ra `model.get_params()` để lấy `n_estimators/max_depth/...`, sau đó thay vào các nhãn `<INSERT_...>`.

## Kết quả thực nghiệm theo từng module

**Quy ước placeholder:** Khi ảnh không cung cấp số cụ thể, báo cáo sẽ ghi rõ **“(không có số trong ảnh)”** và đặt placeholder dạng `<INSERT_...>`. Khi bạn có bảng CSV/log đầy đủ, chỉ cần thay các placeholder bằng số thật là hoàn tất.
...
Lưu ý: **Support không hiển thị trong bảng ảnh**, nên được để placeholder.

| # | Antecedents (rút gọn như notebook, cần thay bằng bản đầy đủ khi có CSV) | Consequents (rút gọn) | Support | Confidence | Lift |
|---:|---|---|---:|---:|---:|
| 1 | Rotational speed_Bin_Low, Process tempe… | Torque_Bin_High, Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_1>` | **0,216463** | **8,168431** |
| 2 | Process temperature_Bin_Medium, Air tempe… | Rotational speed_Bin_Low, Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_2>` | **0,213855** | **8,039678** |
| 3 | Rotational speed_Bin_Low, Process tempe… | Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_3>` | **0,268939** | **7,933315** |
| 4 | Air temperature_Bin_High, Torque_Bin… | Rotational speed_Bin_Low, Process tempe… | (không có số trong ảnh) `<INSERT_SUPPORT_4>` | **0,076487** | **7,498750** |
| 5 | Rotational speed_Bin_Low, Air temperatu… | Process temperature_Bin_High, Status_Fail… | (không có số trong ảnh) `<INSERT_SUPPORT_5>` | **0,095972** | **7,382428** |
| 6 | Rotational speed_Bin_Low, Air temperatu… | Torque_Bin_High, Process temperature… | (không có số trong ảnh) `<INSERT_SUPPORT_6>` | **0,076705** | **7,375437** |
| 7 | Air temperature_Bin_High, Tool wear… | Rotational speed_Bin_Low, Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_7>` | **0,190202** | **7,150441** |
| 8 | Rotational speed_Bin_Low, Air temperatu… | Torque_Bin_High, Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_8>` | **0,189112** | **7,136292** |
| 9 | Rotational speed_Bin_Low, Air temperatu… | Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_9>` | **0,235714** | **6,953224** |
| 10 | Process temperature_Bin_Medium, Air tempe… | Status_Failure | (không có số trong ảnh) `<INSERT_SUPPORT_10>` | **0,225610** | **6,655155** |

**Diễn giải (ngắn gọn, “ăn điểm”):**  
...
- Khi nộp bài, nên xuất file `ket_qua_luat_loi.csv` (đã có trong repo) để điền **support** và tên itemset đầy đủ.
...
|---:|---:|---:|---:|
| 2 | (không có số trong ảnh) `<INSERT_INERTIA_K2>` | (không có số trong ảnh) `<INSERT_SIL_K2>` | (không có số trong ảnh) `<INSERT_DB_K2>` |
| 3 | (không có số trong ảnh) `<INSERT_INERTIA_K3>` | (không có số trong ảnh) `<INSERT_SIL_K3>` | (không có số trong ảnh) `<INSERT_DB_K3>` |
| 4 | (không có số trong ảnh) `<INSERT_INERTIA_K4>` | (không có số trong ảnh) `<INSERT_SIL_K4>` | (không có số trong ảnh) `<INSERT_DB_K4>` |
| 5 | (không có số trong ảnh) `<INSERT_INERTIA_K5>` | (không có số trong ảnh) `<INSERT_SIL_K5>` | (không có số trong ảnh) `<INSERT_DB_K5>` |
| 6 | (không có số trong ảnh) `<INSERT_INERTIA_K6>` | (không có số trong ảnh) `<INSERT_SIL_K6>` | (không có số trong ảnh) `<INSERT_DB_K6>` |

Gợi ý: Silhouette tốt nhất gần 1 và kém nhất -1; Davies–Bouldin càng thấp càng tốt. citeturn1search1turn1search5  
...
|---|---:|---:|---:|---:|---:|
| Random Forest (best) | **1,0000** | **0,9559** | **0,9774** | **0,9842** | (không có số trong ảnh) `<INSERT_PR_AUC_RF>` |
| Decision Tree | (không có số trong ảnh) `<INSERT_PREC_DT>` | (không có số trong ảnh) `<INSERT_REC_DT>` | (không có số trong ảnh) `<INSERT_F1_DT>` | (không có số trong ảnh) `<INSERT_ROC_DT>` | (không có số trong ảnh) `<INSERT_PR_DT>` |
...
**Phần giải thích ngắn (đúng tinh thần “bảo vệ trước giảng viên”):**  
...
| 10% | self_training | **0,7857** | **97,32%** (từ log: 7007/7200) | (không có số trong ảnh) `<INSERT_PSEUDO_ACC_10>` |
| 20% | supervised_only | (không có số trong ảnh) `<INSERT_F1_SUP_20>` | – | – |
| 20% | self_training | (không có số trong ảnh) `<INSERT_F1_ST_20>` | (không có số trong ảnh) `<INSERT_COV_20>` | (không có số trong ảnh) `<INSERT_ACC_20>` |
| 30% | supervised_only | (không có số trong ảnh) `<INSERT_F1_SUP_30>` | – | – |
| 30% | self_training | (không có số trong ảnh) `<INSERT_F1_ST_30>` | (không có số trong ảnh) `<INSERT_COV_30>` | (không có số trong ảnh) `<INSERT_ACC_30>` |
...
|---|---:|---:|---:|
| Random Forest Regressor | **4,3920** | **21,8645** | (không có số trong ảnh) `<INSERT_SMAPE>` |
...
## Hướng dẫn chạy repo và reproducibility
